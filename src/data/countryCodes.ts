/**
 * Country dialling codes for the phone field on the contact form.
 *
 * Only three fields are stored per country: the ISO 3166-1 alpha-2 code, the
 * English name, and the dialling code. The flag is DERIVED from the ISO code
 * at render time (see `flagFor`) rather than stored as an emoji — 200-odd
 * hand-typed flag emoji is 200-odd chances to paste the wrong one, and the
 * regional-indicator maths cannot get it wrong.
 *
 * Names are English only, deliberately. Translating every country into
 * Albanian is a large surface for small mistakes, and the visible part of the
 * closed dropdown is the flag and the code, which need no translation.
 */
...
/**
 * Turns 'XK' into the flag emoji by mapping each letter onto its regional
 * indicator symbol (U+1F1E6 is the indicator for 'A').
 */
...
/** Shown at the top of the dropdown — where deSuisse customers actually are. */
export const PRIORITY_ISO = ['XK', 'AL', 'MK', 'ME', 'RS', 'CH', 'DE', 'AT', 'IT', 'US'];

/** The country a fresh form starts on: the boutique's own country. */
export const DEFAULT_DIAL = '+383';
...
export const PRIORITY_COUNTRIES = PRIORITY_ISO
  .map(iso => COUNTRY_CODES.find(c => c.iso === iso))
  .filter((c): c is CountryCode => Boolean(c));

/** Label shown in the list: flag and code FIRST, so a narrow select that
 *  truncates still shows the part that matters. */
export function codeLabel(c: CountryCode): string {
  return `${flagFor(c.iso)}  ${c.dial}  ${c.name}`;
}
